...
//!
//! Ported from braid_tauri/crates/braid-core/src/fs/debouncer.rs
...
use crate::fs::sync::engine::sync_local_to_remote;
...
impl DebouncedSyncManager {
    ...
    async fn perform_sync(
        ...
        while attempts < 3 {
            match tokio::fs::read_to_string(path).await {
                ...
                Err(e) if e.kind() == std::io::ErrorKind::NotFound => {
                    info!(
                        "[Debouncer] File removed, propagating deletion for {}",
                        url
                    );
        ...
        let content = content.ok_or_else(|| {
            crate::core::BraidError::Fs(format!(
        ...
        sync_local_to_remote(path, url, &parents, original_content, content, state).await
    }
}
