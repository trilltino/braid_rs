...
pub async fn sync_local_to_remote(
    ...
    let _author_email = author_ident.clone().unwrap_or_else(|| "anon".to_string());
    let clean_content = new_content
        .strip_prefix("\u{feff}")
        .unwrap_or(&new_content)
        .to_string();

    let final_body = clean_content.clone();
    ...
    let (request_with_payload, _is_patch) = {
        ...
        if let Some(merge) = merges.get_mut(url) {
            ...
            let patches = crate::fs::local::diff::compute_patches(original, &new_content);
            if patches.is_empty() {
                return Ok(());
            }
