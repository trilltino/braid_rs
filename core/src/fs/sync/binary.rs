...
use crate::fs::common::config::is_binary;
use crate::fs::common::rate_limiter::ReconnectRateLimiter;
...
impl BinarySyncManager {
    ...
    /// 
    /// Takes the meta directory path (e.g., `.braidfs`) for storing temp files and metadata.
    pub fn new(
        rate_limiter: Arc<ReconnectRateLimiter>,
        ...
        Ok(Self {
            ...
            temp_folder: meta_dir.join("temp"),
            meta_folder: meta_dir.join("braid-blob-meta"),
        })
    }
    ...
    pub async fn signal_file_needs_reading(&self, url: &str, fullpath: &Path) -> Result<()> {
        ...
        if needs_upload {
            ...
            if let Some(store) = &self.blob_store {
                store.put(url, data.into(), vec![], vec![], None).await?;
            }
    ...
    fn get_meta_path(&self, url: &str) -> PathBuf {
        let encoded = crate::blob::store::encode_filename(url);
...
impl BinarySyncDb {
    ...
    pub async fn write(&self, _key: &str, data: &[u8]) -> Result<std::fs::Metadata> {
        crate::blob::store::atomic_write(&self.fullpath, data, &self.temp_folder).await
    }
