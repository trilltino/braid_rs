...
use crate::fs::common::config::Config;
...
impl VersionStore {
    /// Load version store from the default location based on config.
    pub async fn load(config: &Config) -> Result<Self> {
        let store_path = config.get_versions_path()?;
        Self::load_from(store_path).await
    }

    /// Load with backward compatibility - tries new location first, then old.
    #[deprecated(since = "0.2.0", note = "Use load(config) instead")]
    pub async fn load_legacy() -> Result<Self> {
