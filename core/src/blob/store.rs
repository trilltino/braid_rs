...
impl BlobStore {
    ...
    ///
    /// For braid-blob, this checks if the given version matches what we have stored.
    /// JS equivalent: version comparison in braid-blob.get()
    pub async fn has_version(&self, key: &str, version: &str) -> bool {
        if let Ok(Some(meta)) = self.get_meta(key).await {
            // Check if the requested version matches our current version
            // For a simple blob store, we only track the current version
            meta.version.iter().any(|v| v.to_string() == version)
        } else {
            false
        }
    }
