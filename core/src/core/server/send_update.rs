...
// =============================================================================
// SendUpdateExt Implementation for Response
// =============================================================================

impl SendUpdateExt for Response {
    fn send_update(&mut self, _update: &Update) -> Result<()> {
        // For now, this is a placeholder - the actual sending is done via IntoResponse
        // In a full implementation, this would modify the response in place
        Ok(())
    }
    ...
    fn send_body(&mut self, body: &[u8]) -> Result<()> {
        *self = Response::builder()
            ...
            .map_err(|e| crate::core::error::BraidError::Http(e.to_string()))?;
        Ok(())
    }

    fn start_subscription(&mut self, subscribe_header: Option<&str>) {
        // Set status 209
        *self = Response::builder()
            .status(StatusCode::from_u16(209).unwrap())
            .header("subscribe", subscribe_header.unwrap_or("true"))
...
/// # JS Parity
///
/// Matches the JS `braidify` helper methods:
/// - `res.sendUpdate(data)` → `send_update()`
/// - `res.startSubscription()` → `start_subscription()`
/// - `res.sendVersion()` → `send_update()` (deprecated alias in JS)
///
/// # Section 2.5 Support
///
/// These helpers enable apps to implement Section 2.5 rules themselves:
/// - App checks `req.version`, `req.parents`, `req.subscribe`
/// - App decides response mode (single, subscription, history range)
/// - App uses these helpers to send appropriate response
///
/// This is the JS approach: provide tools, let app decide logic.
pub trait SendUpdateExt {
    ...
    /// Start a subscription (HTTP 209) response.
    ///
    /// Sets status 209 and required headers for streaming updates.
    /// After calling this, stream updates via `send_update()`.
    ///
    /// # JS Equivalent
    /// ```javascript
    ...
    /// # Section 2.5 Usage
    ///
    /// For "Parents + Subscribe" (send history then subscribe):
    /// ```rust,ignore
    /// if subscribe && parents.is_some() {
    ///     res.start_subscription(Some("true"));
    ///     // Send historical updates
    ///     for update in history { res.send_update(&update)?; }
    ...
    /// # Arguments
    ///
    /// * `subscribe_header` - Value from request's Subscribe header (typically "true")
    fn start_subscription(&mut self, subscribe_header: Option<&str>);
}
...
///
/// ```ignore
/// use braid_http_rs::core::server::UpdateResponse;
/// use braid_http_rs::core::Version;
///
        ...
        if let Some(body) = &self.body {
            ...
            if patches.len() == 1 {
                ...
                for patch in patches {
                    ...
                    // Add extra headers from patch
                    for (key, value) in &patch.extra_headers {
                        patch_headers.push_str(&format!("{}: {}\r\n", key, value));
                    }
                    
                    patch_headers.push_str("\r\n");
