...
pub struct Patch {
    ...
    /// Extra headers for this patch (custom headers beyond Content-Range/Content-Length)
...
impl Patch {
    ...
    /// Create a new patch with extra headers.
    #[must_use]
    pub fn with_headers(
        unit: impl Into<String>,
        range: impl Into<String>,
        ...
        let mut patch = Self::new(unit, range, content);
        patch.extra_headers = headers;
        patch
    }

    /// Add an extra header to this patch.
    #[must_use]
    pub fn with_header(
        mut self,
...
mod tests {
    ...
    fn test_extra_headers() {
        ...
        assert_eq!(patch.extra_headers.get("X-Custom"), Some(&"header-value".to_string()));
        assert_eq!(patch.extra_headers.get("X-Another"), Some(&"another-value".to_string()));
    }

    #[test]
    fn test_with_headers() {
        let mut headers = BTreeMap::new();
        headers.insert("X-Id".to_string(), "123".to_string());
        
        let patch = Patch::with_headers("json", ".data", "test", headers);
        
        assert_eq!(patch.extra_headers.get("X-Id"), Some(&"123".to_string()));
    }
}
