...
impl BraidClient {
    ...
    async fn fetch_with_retries(&self, url: &str, request: BraidRequest) -> Result<BraidResponse> {
        ...
        let mut retry_state = crate::core::client::retry::RetryState::new(retry_config.clone());

        loop {
            // Call on_fetch callback before each attempt
            request.notify_on_fetch(url);
            
            // Resolve parents (either static or from callback)
            let mut request_with_parents = request.clone();
            if let Some(parents) = request.resolve_parents() {
                request_with_parents.parents = Some(parents);
            }
            
            // Set cache-control header to prevent browser caching (JS: cache: 'no-cache')
            if !request_with_parents.extra_headers.contains_key("cache-control") {
                request_with_parents.extra_headers.insert(
                    "cache-control".to_string(),
                    "no-cache, no-store".to_string(),
                );
            }

            match self.fetch_internal(url, &request_with_parents).await {
                Ok(response) => {
                    // Notify on_res callback if set
                    if let Some(ref retry) = request.retry {
                        retry.notify_response(&response);
                    }
                    ...
                    if (400..600).contains(&status) {
                        ...
                        if let Some(ref retry) = request.retry {
                            if !retry.should_retry_response(&response) {
                                return Ok(response);
                            }
                        }
                        ...
                        let status_text = response.headers.get("status-text").cloned();

                        match retry_state.should_retry_status_with_text(
                            status,
    ...
    pub async fn fetch_multiplexed(
        ...
        // Call on_fetch callback before multiplexed fetch
        request.notify_on_fetch(url);
        ...
        if initial_response.status == 293 {
            ...
            while let Ok(chunk) = rx.recv().await {
                // Call on_bytes callback for progress tracking
                request.notify_on_bytes(&chunk);
