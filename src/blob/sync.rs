...
impl BlobSyncClient {
    ...
    pub fn new(config: BlobSyncConfig) -> (Self, mpsc::Receiver<BlobUpdate>) {
    ...
    pub async fn sync<F, W, D>(&self, on_read: F, on_write: W, on_delete: D) -> Result<()>
    where
        F: Fn(
                &str,
            ) -> std::pin::Pin<
            ...
            + Sync,
    {
        ...
        if !response.status().is_success() {
            return Err(anyhow::anyhow!("Blob sync failed: {}", response.status()));
        }
    ...
    pub async fn put(
        ...
        if !response.status().is_success() {
            return Err(anyhow::anyhow!("Blob put failed: {}", response.status()));
        }
    ...
    pub async fn delete(&self, key: &str) -> Result<()> {
        ...
        if !response.status().is_success() && response.status().as_u16() != 404 {
            return Err(anyhow::anyhow!("Blob delete failed: {}", response.status()));
        }
