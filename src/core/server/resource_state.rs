...
mod tests {
    ...
    fn test_apply_update() {
        let manager = ResourceStateManager::new();
        let result = manager.apply_update("doc1", "hello", "alice", None);
        assert!(result.is_ok());
    ...
    fn test_concurrent_updates() {
        let manager = ResourceStateManager::new();
        let _ = manager.apply_remote_insert("doc1", "alice", 0, "hello", None);
        let _ = manager.apply_remote_insert("doc1", "bob", 5, " world", None);
