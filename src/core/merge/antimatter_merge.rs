...
impl AntimatterMergeType {
    /// Create a new Antimatter merge type with the given peer ID.
...
impl MergeType for AntimatterMergeType {
    ...
    fn clone_box(&self) -> Box<dyn MergeType> {
        ...
        let new_merge = AntimatterMergeType::new(&self.crdt.id);
        Box::new(new_merge)
    }
}
...
mod tests {
    ...
    fn test_antimatter_creation() {
        let merge = AntimatterMergeType::new("alice");
        assert_eq!(merge.name(), "antimatter");
    }

    #[test]
    fn test_antimatter_initialize() {
        let mut merge = AntimatterMergeType::new("alice");
